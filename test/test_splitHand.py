...
Jack_spades = getCard("Hearts", 10)
King_hearts = getCard("Hearts", 10)
...
def blackjackGame():
  ...
  while betting:
    ###NORMAL GAMELOOP####
      ...
      player_name.addCard(King_hearts)
      player_name.addCard(Jack_spades)
      ...
      if bj.blackjackDecider(house, player_name, betting_box, dealer) == True:
        ...
        if player_name.splitHand() == True:
          ...
          hand_num = 1
          while len(player_name.hand) > 0:

            while player_name.splitHand() == True and len(player_name.completed_hands)+len(player_name.hands_lst) < 3:
              player_name.addCard(Jack_spades)
